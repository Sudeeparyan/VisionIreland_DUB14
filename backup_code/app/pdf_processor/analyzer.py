            ...
            for attempt in range(MAX_RETRIES):
                try:
                    ...
                    if is_retryable and attempt < MAX_RETRIES - 1:
                        ...
                        print(
                            f"Page {page_number}: API overloaded, retrying in {delay:.1f}s (attempt {attempt + 1}/{MAX_RETRIES})"
                        )
            ...
            for attempt in range(MAX_RETRIES):
                try:
                    ...
                    if is_retryable and attempt < MAX_RETRIES - 1:
                        ...
                        print(
                            f"Panel description: API overloaded, retrying in {delay:.1f}s (attempt {attempt + 1}/{MAX_RETRIES})"
                        )
                        await asyncio.sleep(delay)
                    else:
                        raise
