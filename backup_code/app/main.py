...
async def process_comic_pdf(file_path: Path, session_id: str) -> Dict:
    ...
    try:
        ...
        # Only analyze the FIRST page with AI during upload (to avoid rate limits)
        # Other pages will be analyzed on-demand when the user navigates to them
        for idx, page in enumerate(pages):
            if idx == 0 and page.get("image_base64"):  # AI analyze only first page
                try:
                ...
                # For all other pages, use text-only fallback (will be analyzed on-demand)
                analysis_results.append(
                    {
                        "success": True,
                        "page_number": page["page_number"],
    ...
    # Get first page narration for immediate audio playback
    first_page_data = None
    comic_pages = comic_dict.get("pages", [])
    if comic_pages:
        for page in comic_pages:
            if page.get("page_number") == 1:
                first_page_data = page
                break
        if not first_page_data and comic_pages:
            first_page_data = comic_pages[0]

    first_page_narration = ""
    if first_page_data:
        first_page_narration = first_page_data.get("narration_script", "")
        if not first_page_narration:
            # Build narration from available data
            parts = []
            if first_page_data.get("setting"):
                parts.append(first_page_data["setting"])
            if first_page_data.get("action_summary"):
                parts.append(first_page_data["action_summary"])
            first_page_narration = " ".join(parts)
    ...
    return {
        ...
        "first_page": {
            "narration": first_page_narration,
            "setting": first_page_data.get("setting", "") if first_page_data else "",
            "characters": (
                first_page_data.get("characters_present", []) if first_page_data else []
            ),
        },
    }
