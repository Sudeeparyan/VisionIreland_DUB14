...
async function initAudio() {
  try {
    ...
    // Add click listener to resume audio context (browser autoplay policy)
    setupAudioResume();
  } catch (error) {
...
/**
 * Resume AudioContext after user interaction (required by browser autoplay policy)
 */
async function resumeAudioContext() {
  if (audioContext && audioContext.state === "suspended") {
    try {
      ...
      console.log(
        "AudioContext resumed successfully, state:",
        audioContext.state
      );
    } catch (error) {
...
/**
 * Setup listeners to resume audio on first user interaction
 */
...
async function handleServerMessage(message) {
  ...
  if (message.mime_type === "audio/pcm" && audioPlayerNode) {
    ...
    console.log(
      "Audio data sent to player, buffer size:",
...
async function uploadFile(file) {
  ...
  // Resume audio context on user interaction (file upload)
  await resumeAudioContext();
...
function handleComicLoaded(data) {
  ...
  // Automatically start comic-based audio narration
  startComicNarration(data);
}

/**
 * Automatically start comic-based audio narration after upload
 * Sends a request to AI to narrate the comic in an engaging, comic-style way
 */
function startComicNarration(data) {
  // Wait for WebSocket to be ready
  if (!state.websocket || state.websocket.readyState !== WebSocket.OPEN) {
    // Retry after a short delay if WebSocket isn't ready
    setTimeout(() => startComicNarration(data), 500);
    return;
  }

  showTypingIndicator();

  // Build a detailed prompt for comic-style narration
  const comicTitle = data.title || "this comic";
  const author =
    data.author && data.author !== "Unknown" ? ` by ${data.author}` : "";
  const pageCount = data.page_count || state.totalPages;
  const synopsis = data.synopsis || "";

  // Get first page content if available
  let firstPageContent = "";
  if (data.first_page) {
    if (data.first_page.narration) {
      firstPageContent = `\n\nFirst page content: ${data.first_page.narration}`;
    }
    if (data.first_page.setting) {
      firstPageContent += `\nSetting: ${data.first_page.setting}`;
    }
    if (data.first_page.characters && data.first_page.characters.length > 0) {
      firstPageContent += `\nCharacters: ${data.first_page.characters.join(
        ", "
      )}`;
    }
  }

  // Request comic-style audio narration from AI
  const narrationRequest = `
I just uploaded a comic book called "${comicTitle}"${author} with ${pageCount} pages.
${synopsis ? `Synopsis: ${synopsis}` : ""}${firstPageContent}

Please give me an exciting, comic-book style audio introduction and then narrate the first page with:
- Dramatic voice acting and sound effects (POW! BAM! WHOOSH!)
- Character voices if there are dialogues
- Vivid scene descriptions like a radio drama
- Comic book style excitement and energy

Make it fun and engaging like listening to an audio comic book adventure! Start now!
`.trim();

  sendToServer({
    mime_type: "text/plain",
    data: narrationRequest,
    role: "user",
  });

  addMessage("🎭 Starting comic narration...", "user");
  announce(`Loading ${comicTitle}. Get ready for an audio comic adventure!`);
}
